...
function FollowerDetailScreen(props) {
  // console.log(">>>>>>>>>>>>>>>>>>", props.route.params.navigation.navigate, "this is props in followe detail screen")
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setglobalState(await JSON.parse(data));
      } catch (e) {
        console.log(e);
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await AsyncStorage.getItem("quinkUser");
        setglobalState(await JSON.parse(data));
        const curruser = JSON.parse(data)
        const foll = await axios.get(
          `${BACKEND}/user/follwers/followings/${curruser?._id}`
        );
  ...
  const fetchfollow = () => {
    if (popularSelected == true) {
          ...
          <View style={styles.container}>
            ...
            <View style={{ marginLeft: 15 }}>
              ...
              <Text style={styles.text}>
                {partner?.firstName ? partner?.firstName : "first_name"}
              </Text>
            </View>
      ...
      return following?.map((partner) => {
        // console.log(partner.userName, "this is partner")
        return (
          <View style={styles.container}>
            <TouchableOpacity
              onPress={() =>
